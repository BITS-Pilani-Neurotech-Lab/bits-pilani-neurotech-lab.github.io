...
export default function Pieces() {
  ...
  return (
    <SubpageLayout>
      ...
      {/* Articles Stack Layered Above Background */}
      <main className="relative z-10 w-full flex flex-col gap-3 md:gap-[15px]">
        {piecesData.map((piece) => (
          <article
            key={piece.id}
            onClick={() => handlePieceClick(piece)}
            className={`group relative w-full min-h-[90px] md:h-[100px] ${piece.bgColor} rounded-[20px] md:rounded-[28px] p-4 px-6 md:p-6 md:px-8 flex items-center justify-between cursor-pointer overflow-hidden transition-all duration-300 hover:scale-[1.01] shadow-md`}
          >
            {/* Title & Author Stack */}
            <div className="flex flex-col z-0">
              <h2
                className={`text-lg md:text-2xl font-bold tracking-tight ${piece.textColor} transition-colors duration-300 ${
                  piece.isDarkTile
              ...
              <p
                className={`text-xs md:text-sm font-medium opacity-80 ${piece.textColor}`}
              >
                by {piece.author}
              </p>
            </div>
            ...
            <div className="absolute inset-0 z-10 pointer-events-none rounded-[20px] md:rounded-[28px] bg-white/0 backdrop-blur-none transition-all duration-300 group-hover:bg-white/20 group-hover:backdrop-blur-[6px] group-hover:border group-hover:border-zinc-500/60 group-hover:ring-1 group-hover:ring-white/40 group-hover:shadow-lg" />
          </article>
        ))}
      </main>
